			...
			<motion.section variants={VARIANTS_SECTION} transition={TRANSITION_SECTION}>
				...
				<div className="flex flex-col space-y-2">
					{WORK_EXPERIENCE.map((job) => (
						<a className="relative overflow-hidden rounded-2xl bg-zinc-300/30 p-[1px] dark:bg-zinc-600/30" href={job.link} target="_blank" rel="noopener noreferrer" key={job.id} >
							<Spotlight
			...
			<motion.section variants={VARIANTS_SECTION} transition={TRANSITION_SECTION}>
				...
				<div className="flex items-center justify-start space-x-3">
					{SOCIAL_LINKS.map((link) => (
						<MagneticSocialLink key={link} link={getData(link)}>
							{link}
						</MagneticSocialLink>
					))}
				</div>
